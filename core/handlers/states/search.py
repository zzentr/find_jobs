...
from main import all_specialties, all_vacancies
...
from core.synchronous_functions import create_dict_with_vacancies, create_message_with_vacancies, check_correct_area
...
async def choose_speciality(message: Message, state: FSMContext):
    """Проверяем правильность специальности"""
...
async def choose_salary(message: Message, state: FSMContext):
    """Проверяем правильность заработной платы"""
    salary = message.text
...
async def choose_area(message: Message, state: FSMContext):
    """Проверяем правильность страны/района/города"""
...
async def send_more_vacancies(message: Message, state: FSMContext):
     """Обрабатываем сообщения при отправке вакансий"""
     text = message.text
     ...
     if text == 'Показать ещё':
        if all_vacancies.get(message.from_user.id):
          ...
          await message.answer('Выберите действие на клавиатуре или отправьте мне номер вакансии чтобы увдиеть её подробнее!')
